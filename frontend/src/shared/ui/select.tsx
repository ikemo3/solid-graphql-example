...
const SelectTrigger: Component<SelectPrimitive.SelectTriggerProps> = (
  ...
  return (
    <SelectPrimitive.Trigger
      class={cn(
        "border-input ring-offset-background placeholder:text-muted-foreground focus:ring-ring flex h-10 w-full items-center justify-between rounded-md border bg-transparent px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",
        props.class,
      )}
      {...rest}
    >
...
const SelectItem: Component<SelectPrimitive.SelectItemProps> = (props) => {
  ...
  return (
    <SelectPrimitive.Item
      class={cn(
        // text-sm → text-base に変更(iOSのSafari対応)
        "focus:bg-accent focus:text-accent-foreground relative mt-0 flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-base outline-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        props.class,
      )}
      {...rest}
    >
