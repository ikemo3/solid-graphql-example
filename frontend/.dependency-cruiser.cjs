...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      to: {
        ...
        pathNot: [
          "node_modules/@types/",
          "/solid-devtools/", // solid-devtools は開発時のみに使うように設定されているため除外
        ],
      },
    },
    {
    ...
    /*
       ...
       以上より、次のことが言える
       - pagesはentities, features, widgetsが使える
       - widgetsはentities, featuresが使える

       これを踏まえて、以下のルールを設定する
       - entitiesはpages, widgetsからしか使えない
       - featuresはpages, widgetsからしか使えない
       - widgetsはpagesからしか使えない
    */
    {
      name: "not-to-app",
      severity: "error",
    ...
    },
    {
      ...
      severity: "error",
      from: {
        // pagesはネスト可能なので、pages以下のファイルはエラーにしない
        pathNot: [
          "^src/app",
          "^src/pages",
          // テスト用
    ...
    },
    {
      ...
      severity: "error",
      from: {
        // widgetsはネスト不可能なので、widgets以下のファイルもエラーにする
        pathNot: [
          "^src/pages",
          // テスト用
          "^tests/widgets",
    ...
    {
      name: "not-to-features",
      severity: "error",
      from: {
        // featuresはネスト不可能なので、features以下のファイルもエラーにする
        pathNot: [
          "^src/app", // AlertDialog などは app から使う
          "^src/pages",
          "^src/widgets",
          // テスト用
          "^tests/features",
    ...
    {
      // shared/api 以下を直接呼び出さない
      name: "not-to-shared-api",
      severity: "error",
      from: {
        pathNot: ["^src/shared", "^tests/"],
      },
      to: {
        path: ["^src/shared/api"],
    ...
    {
      // shared/lib 以下を直接呼び出さない
      name: "not-to-shared-lib",
      severity: "error",
      from: {
        pathNot: ["^src/shared", "^tests/"],
      },
      to: {
        path: ["^src/shared/lib"],
        pathNot: ["^src/shared/lib/index.ts"],
      },
    },
    {
      // shared/ui 以下を直接呼び出さない
      name: "not-to-shared-ui",
      severity: "error",
      from: {
        pathNot: ["^src/shared", "^tests/"],
      },
  ...
  options: {
    ...
    */
    doNotFollow: {
      path: ["node_modules", "coverage", "dist", "types"],
    },
    ...
    */
    ...
    reporterOptions: {
      dot: {
        ...
        filters: {
          exclude: {
            path: ["src/router.ts", "types/vite-env.d.ts"],
          },
        },
...
// generated: dependency-cruiser@16.0.0 on 2024-01-04T13:51:29.521Z
